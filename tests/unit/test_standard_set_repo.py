...
@pytest.mark.asyncio
async def test_create_existing_set(repo, standard_set_data):
    # Setup
    ...
    repo.standards_collection.delete_many.assert_called_once_with(
        {"standard_set_id": str(existing_id)}
    )


@pytest.mark.asyncio
async def test_create_new_set(repo, standard_set_data):
    # Setup
    standard_set = StandardSetCreate(**standard_set_data)
    ...
    repo.collection.find_one_and_replace = AsyncMock(return_value=created_doc)
...
@pytest.mark.asyncio
async def test_create_with_error(repo, standard_set_data):
    # Setup
    standard_set = StandardSetCreate(**standard_set_data)
...
async def test_get_all_with_items(repo):
    # Setup
    mock_items = [{"_id": ObjectId(), "name": "Set 1"}, {
        "_id": ObjectId(), "name": "Set 2"}]
    repo.collection.find = Mock(
        return_value=AsyncIteratorMock(mock_items.copy()))
...
async def test_update_success(repo, standard_set_data):
    ...
    repo.collection.replace_one = AsyncMock(
